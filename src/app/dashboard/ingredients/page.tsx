...
export default async function IngredientsPage({
  ...
  const ingredientsResult = await getIngredientsByBakery(user.bakeryId, search);
  ...
  return (
    <>
      <SetPageHeader
        ...
        sticky
        centerContent={<SearchFilterInput placeholder="Search ingredients..." />}
        actions={
      ...
      {ingredients.length === 0 ? (
        search ? (
          <EmptyState
            ...
            description={`No ingredients matching "${search}". Try a different search term.`}
          />
        ) : (
          <EmptyState
            icon={Package}
        ...
        <div className="space-y-6">
          ...
          <IngredientsTable key={search || ''} ingredients={serializedIngredients} />
        </div>
      )}
    </>
  );
